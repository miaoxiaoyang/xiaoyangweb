...
CORS 可以理解成是 即可以同步，也可以异步的Ajax。
fetch 是一个比较全新的API，用来实现CORS通信，用法如下：
...
fetch（'/some/url/',{
      method:'get',
}）.then(function (response)){//类似于ES6中的promise
...
4,hash
url的 # 后面的内容就叫 hash。 hash的改变，页面不会刷新，这就是用 hash 做跨域通信的基本原理。
补充：url的？后面的内容叫 Search。Search的改变，会导致页面刷新，因此不能做跨域通信。
...
```
5.postMessage（）方法，这个是在h5方法中新增的方法
