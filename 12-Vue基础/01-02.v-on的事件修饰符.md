...
注意：
   情景：当父标签和子标签都存在点击事件，就会出现点击子标签，父标签也会随之出现，
   作用：这时候，如果只想让子标签出现，就添加一个 事件修饰符 stop, 阻止冒泡，这里有个小技巧就是 添加的时候  .stop是阻止父亲事件 :stop 是阻止其本身了子标签；
...
@@  这里运用 超链接来 处理，超链接 默认有跳转行为，那我可以通过事件修饰符，.prevent 阻止这种跳转行为。
```html
<!DOCTYPE html>
<html lang="en">
...
.self也是冒泡事件，但是两者的区别是 前者 .stop 是阻止整个冒泡行为，后者是只能阻止自己身上的冒泡行为。
