...
全局组件的定义和注册有三种方式，我们接下来讲一讲。(3种)

### 写法一

写法一：使用Vue.extend方法定义组件，使用 Vue.component方法注册组件。

@@ 使用Vue.extend方法定义组件，使用Vue.component方法注册组件。
...
<head>
     ...
     <meta http-equiv='X-UA-Compatible' content="ie=edge">
     <title>Document</title>
     <script src="vue2.5.16.js"></script>//引入vue框架
<head>
<body>
   <div id="app">
      <account> </account> //3, 第三步 如果要想使用组件，直接把组件的名称，以HTML标签的形式，引入到页面中，即可。
   <div>
   <script>
      var myAccount=Vue.extend({//第一步通过使用 Vue.extend 定义组件
         template:'<div><h2>登陆页面</h2> <h3>注册页面</h3></div>'//通过 template属性，指定了组件要展示的HTML结构，template是Vue 中的关键字，不能改
      })；//第二步：使用Vue.component 注册组件，
      Vue.component('account',myAccount);//第一个参数是组件的名称（标签名），第二个参数是模版对象。
...
```
<my-component><my-component> //在html中 使用 需要把大写改成小写
...
<body>
    ...
    <script>
        Vue.component('account'{
             template:'<div><h2>登陆页面<h2> <h3>注册页面<h3></div>'         
        });
...
<body>
   ...
   <script>
      Vue.component({//注册中放进去定义，在标签中放入定义内容
          template:'#myAccount' 
      });
      new Vue({
        el:'#app'
    ...
    <script>
        ...
        new Vue({
            ...
            components: { // 定义、注册Vue实例内部的私有组件
                myLogin: { // 组件的名称
                    template: '<h3>这是私有的login组件</h3>' //这个是配置 html 的
                }
            }
...
<div id="app">
   <acount></acount>
<script>
  Vue.component('account',{//注册中定义
       template:'#myAccount',//注意组件中的data，不再是个对象，而且这个方法内部还必须返回一个对象才行。
       data:function(){
          return{
             myData:'smyhvae'
          }
...
注意的是：代码中，我们在account 组件中添加的data 和 methods,其 作用域 只限于 'account'组件里，保证独立性。只能在account中 
在为组件添加数据时，data不再是对象了，而是一个function,而且还要通过 return 的形式进行返回；否则，页面上是无法看到效果的，通过 function返回对象的形式来定义data，作用是；
上方代码中，组件<account> 被调用来两次（不像是根组件那样只能调用一次），但是每个组件里的数据 myData 是 各自独立的，不产生冲突。
换成言之，通过函数返回的对象的目的，是为了让每个组件都有自己 独立的数据存储，而不应该共享一套数据。
...
例子中，将组件<counter>调用了两次，由于dataObj是全局对象，导致两个组件实例都可以共享这个 dataObj数据。于是，我们点击任何一个组件实例的按钮，都可以让count数据加1。
...
这个例子实际上跟之前把对象数据直接放在 data:function() 中一样，data函数里会return一个 新开辟 的对象数据，只是不用再把对象封装出去共享了，这个作者肯定蒙蔽了又，介绍了两回了
...
```
注意：这里就是整个是   <component :is="comName"></component> 利用这个标签来切换不同的组件。
