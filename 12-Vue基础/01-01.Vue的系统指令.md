...
<script>
    //new出来的对象就是 MVVM中的 View Module(调度者)
    var myVue=new Vue({
        el:'#div1',//当前vue对象将接管上面的div1区域
        data:{//data就是MVVM中的module
          name:'smyhvae'
        }
    });
</script>
...
<body>
    <div id="app">
        <span v-cloak>content:{{name}}</span>
    </div>
...
@@作用：v-cloak:保持和元素实例的关联，直到结束编译后自动消失。具体操作就是 和 css 规则一起用的时候，能够 解决差值表达式闪烁的问题 （即：可以隐藏未编译的标签直到实例准备完毕）。
实际情况就是：比如说 {{name}} 这个内容，在网速很慢的情况下，一开始会直接显示{{name}} 这个内容，等网络加载完毕，才会显示 smyhvase
...
<v-text>
<span v-text="name"></span>
区别：1，没有闪烁问题 2，插值表达式只会替换自己的这个占位符，并不会把整个元素的内容清空。v-text 会覆盖 元素中 原本的内容。
...
<script>
   var myVue=new Vue({
     ...
     //注意的是，下方这个methods是vue中定义方法的关键字，不能改
     //这个 methods 属性中定义了当前vue 实例所有的可用的方法，此时change 就是自定义的属性方法，点击事件
     methods:{
       change:function(){
         this.name+='1';
       }
...
注意：如果我们在定时器中直接使用this，这个this指向的是 window,为了解决这个问题，我们是通过_this 来解决这个问题
...
VM实例，只关心data数据操作，不需要考虑如何重新渲染DOM页面，减少DOM操作
